...
function searchReults() {
  var linkGraphHolder = document.createElement("div");
  linkGraphHolder.id = "link-graph"
  document.getElementById("link-graph-stuff").append(linkGraphHolder);
  ...
  if (query != "") {
    linkData = linkData.filter( function(entry) {
      return (entry.source.toLowerCase().replace(/\s+/g, "")).includes(query.toLowerCase.replace(/\s+/g, "")) || (entry.target.toLowerCase().replace(/\s+/g, "")).includes(query.toLowerCase.replace(/\s+/g, ""));
    });
  }
  var usedNodeIds = {};
  for (let l of linkData) {
    usedNodeIds[l.source] = true;
    usedNodeIds[l.target] = true;
  }
  nodeData = nodeData.filter( function(entry) {
    return usedNodeIds[entry.id] ? true : false;
  });
  console.log(nodeData);
  console.log(linkData);
}

// function drawSearchableGraph() {
//   document.getElementById("link-graph").remove();
//   var linkGraphHolder = document.createElement("div");
//   linkGraphHolder.id = "link-graph"
//   document.getElementById("link-graph-stuff").append(linkGraphHolder);
//
...
//   if (query != "") {
//     linkData = linkData.filter( function(entry) {
//       return (entry.source.toLowerCase().replace(/\s+/g, "")).includes(query) || (entry.target.toLowerCase().replace(/\s+/g, "")).includes(query);
//     });
//   }
//   var usedNodeIds = {};
//   for (let l of linkData) {
